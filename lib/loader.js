...
class Loader {
  ...
  _load( sourcePath, targetPath, inject = false ) {
    ...
    // add dependency and check for cycles
    this._setComponentDependency( sourcePath, targetPath, inject );
  ...
  _setComponent( path, instance = null ) {
    ...
    if ( !this._componentInstances[path] ) {
  ...
  _setComponentDependency( sourcePath, targetPath, inject = false ) {

    // because inject mode reverse dependency, we swap the
    // dependent and dependency if inject == true
    const dependent = inject ? targetPath : sourcePath;
    const dependency = inject ? sourcePath : targetPath;

    if ( !this._components.hasOwnProperty( dependent ) ) {

      this._components[dependent] = {
        path: dependent,
        instance: null,
        initialized: false,
        dependencies: []
    ...
    this._components[dependent].dependencies.push( dependency );

    // this still checks dependencies in normal direction, even if inject
    if ( this._createsCycle( dependent, dependency, inject ) ) {
      throw new Error( 'dependency cycle created from ' + sourcePath + ' to ' + targetPath );
    }
  ...
  _createsCycle( sourcePath, targetPath, inject = false ) {
    ...
    // console.log( '_createsCycle _instantiateStack', this._instantiateStack );
    // console.log( '_createsCycle sourcePath, targetPath', sourcePath, targetPath );

    // if a something depends on a component that is still instantiating, its a cycle.
    if ( this._instantiateStack.indexOf( inject ? sourcePath : targetPath ) > -1 ) {
      // console.log( 'target instantiating', targetPath, this._instantiateStack );
      return true;
    }
    ...
    component.dependencies.forEach( ( dependencyPath ) => {
      ...
      cycleExists = this._createsCycle( sourcePath, dependencyPath, inject );
